/*
 * Live AI Manager
 * Background management of Live AI sessions - Supports Siri and Shortcuts without unlocking the phone
 */
...
class LiveAIManager: ObservableObject {
    ...
    func startLiveAISession() async {
        ...
        do {
            ...
            // Wait for connection success (up to 10 seconds)
            var connectWait = 0
            ...
            // 6. Start recording directly (don't play TTS to avoid audio session conflicts)
    ...
    private func configureAudioSessionForBackground() throws {
        ...
        // Deactivate first then reactivate to ensure clean state
        do {
    ...
    private func setupOmniCallbacks() {
        ...
        omniService.onFirstAudioSent = { [weak self] in
            Task { @MainActor in
                print("✅ [LiveAIManager] Received first audio sent callback, enabling image sending")
