...
class LiveTranslateViewModel: ObservableObject {
    ...
    @Published var currentOriginal = ""          // Current original text (not supported yet, reserved field)
    @Published var streamingTranslation = ""     // Streaming translation segments
    ...
    /// Glasses microphone is suitable for translating your own speech, iPhone microphone is suitable for translating other's speech
    ...
    init() {
        ...
        let savedTarget = UserDefaults.standard.string(forKey: "translate_target_language") ?? "ro"
        self.targetLanguage = TranslateLanguage(rawValue: savedTarget) ?? .ro
    ...
    func startRecording() {
        ...
        // If image enhancement is enabled, start periodic image sending
    ...
    func swapLanguages() {
        // Can only swap when both languages support audio output
    ...
    private func setupCallbacks() {
        ...
        translateService?.onAudioDone = { [weak self] in
            DispatchQueue.main.async {
                print("🔊 [TranslateVM] Audio playback completed")
            }
        }
