...
class OmniRealtimeService: NSObject {
    ...
    // Dynamically get WebSocket URL based on user's region settings (Beijing/Singapore)
    ...
    private let minChunksBeforePlay = 2 // Start playing after receiving first 2 chunks
    ...
    private func stopPlaybackEngine() {
        ...
        playerNode?.stop()
        playerNode?.reset()  // Clear all buffers in queue
        playbackEngine.stop()
    ...
    func connect() {
        ...
        print("🔌 [Omni] Connecting to WebSocket: \(urlString)")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            print("⚙️ [Omni] Configuring session")
    ...
    private func configureSession() {
        // Get the voice and prompts based on the current language settings.
    ...
    private func handleServerEvent(_ jsonString: String) {
        ...
        DispatchQueue.main.async {
            ...
            case OmniServerEvent.inputAudioBufferSpeechStarted.rawValue:
                print("🎤 [Omni] Speech detected started")
                self.onSpeechStarted?()

            case OmniServerEvent.inputAudioBufferSpeechStopped.rawValue:
                print("🛑 [Omni] Speech detected stopped")
            ...
            case OmniServerEvent.responseAudioTranscriptDone.rawValue:
                ...
                if text.isEmpty {
                    print("⚠️ [Omni] AI response complete but done event has no text field (use accumulated delta)")
                } else {
                    print("✅ [Omni] Complete AI response: \(text)")
                }
                // Always call callback, even if text is empty, let ViewModel use accumulated chunks
                ...
                if let base64Audio = json["delta"] as? String,
                   let audioData = Data(base64Encoded: base64Audio) {
                    ...
                    if !self.isCollectingAudio {
                        ...
                        // Clear any old buffers that might be lingering in playerNode queue
                        if self.isPlaybackEngineRunning {
                            // Important: reset will disconnect playerNode, need full reinitialization
                            self.stopPlaybackEngine()
                            self.setupPlaybackEngine()
                            self.startPlaybackEngine()
                            self.playerNode?.play()
                            print("🔄 [Omni] Playback engine reinitialized")
                        }
                    }

                    self.audioChunkCount += 1

                    // Streaming playback strategy: collect a few chunks first then stream
                    if !self.hasStartedPlaying {
                        // Before first playback: collect first
                        self.audioBuffer.append(audioData)

                        if self.audioChunkCount >= self.minChunksBeforePlay {
                            // Collected enough chunks, start playing
                            self.hasStartedPlaying = true
                            self.playAudio(self.audioBuffer)
                            self.audioBuffer = Data()
                        }
                    } else {
                        // Already playing: schedule each chunk directly, AVAudioPlayerNode will queue automatically
...
extension OmniRealtimeService: URLSessionWebSocketDelegate {
    func urlSession(_ session: URLSession, webSocketTask: URLSessionWebSocketTask, didOpenWithProtocol protocol: String?) {
        print("✅ [Omni] WebSocket connected, protocol: \(`protocol` ?? "none")")
    }
