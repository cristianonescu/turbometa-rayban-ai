...
class QuickVisionStorage {
    ...
    private let maxRecords = 100 // Maximum 100 records
    ...
    func saveRecord(_ record: QuickVisionRecord) {
        ...
        if let encoded = try? JSONEncoder().encode(records) {
            ...
            print("💾 [QuickVisionStorage] Record saved successfully: \(record.id), total: \(records.count)")
        } else {
    ...
    func deleteRecord(_ id: UUID) {
        ...
        if let encoded = try? JSONEncoder().encode(records) {
            userDefaults.set(encoded, forKey: recordsKey)
            print("🗑️ [QuickVisionStorage] Record deleted successfully: \(id)")
        }
    }
