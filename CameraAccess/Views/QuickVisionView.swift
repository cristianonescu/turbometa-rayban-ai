/*
 * Quick Vision View
 * Quick image recognition interface - one-click photo recognition
 */
...
struct QuickVisionView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            // Wait for device connection (max 2 seconds)
            var deviceWait = 0
            ...
            // Auto-start image recognition (includes start stream, capture photo, stop stream, recognize, TTS)
    ...
    private var videoPreviewSection: some View {
        ZStack {
            // Prioritize QuickVisionManager saved photo (won't be cleared when stream stops)
            // Then streamViewModel photo, finally video stream
            if let photo = quickVisionManager.lastImage {
                // Show QuickVisionManager saved photo
                Image(uiImage: photo)
                    .resizable()
                    .aspectRatio(contentMode: .fit)
                    .cornerRadius(AppCornerRadius.lg)
            } else if let photo = streamViewModel.capturedPhoto {
                // Show streamViewModel photo
                Image(uiImage: photo)
                    .resizable()
                    .aspectRatio(contentMode: .fit)
                    .cornerRadius(AppCornerRadius.lg)
            ...
            // Processing overlay (show only when image exists)
